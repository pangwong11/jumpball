...
def get_games(date):
    ...
    soup = make_soup(ESPN_URL + 
        "/nba/scoreboard?date={0}".format(date))
...
if __name__ == '__main__':
    ...
    parser.add_argument('-s', '--start-date', action='store', help='date (YYYYMMDD)', dest="s_date", required=True)
    parser.add_argument('-e', '--end-date', action='store', help='date (YYYYMMDD)', dest="e_date", required=True)
            ...
            for quarter in ['1','2','3','4']:    #4 quarters in a game
                ...
                try:
                    print "Writing data for game: {0}".format(game_id)
                            ...
                            #my file is at ./nba-play-data/ as you can see below.
                    with open("./nba-play-data/" + str(d.date()) + "_" + game_id + ".psv", write_style) as f:
