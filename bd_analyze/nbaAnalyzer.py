...
#for i in range(1,2):
new_team_data = newData_array[labels.ravel()==0]
    #print new_team_data
pyplot.scatter(new_team_data[:,0],new_team_data[:,1],marker = '^',s = new_team_data[:,2]*1000,c = color,cmap = pyplot.get_cmap('Spectral'))

team_test = teamData_2[:2]
for label, x, y in zip(team_test,newData_array[:,0],newData_array[:,1]):
    #print zip(teamData_2[:3],newData_array[:,0],newData_array[:,1])
    print label
    print (label)
    print x
    print y
...
ret, results, neighbours ,dist = knn.find_nearest((new_team_data).astype(np.float32), 3)
